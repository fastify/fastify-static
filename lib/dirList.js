...
const dirList = {
  ...
  send: async function ({ reply, dir, options, route, prefix, dotfiles }) {
    if (reply.request.query.format === 'html' && typeof options.render !== 'function') {
      throw new FST_STATIC_INVALID_OPTION_VALUE('list.render', 'must be a function and is required with the URL parameter `format=html`')
    }
  ...
  validateOptions: function (options) {
    ...
    if (Array.isArray(options.root)) {
      return new FST_STATIC_MULTIROOT_LIST_CONFLICT()
    }

    if (options.list.format && options.list.format !== 'json' && options.list.format !== 'html') {
      return new FST_STATIC_INVALID_OPTION_VALUE('list.format', 'must be json or html')
    }
    if (options.list.names && !Array.isArray(options.list.names)) {
      return new FST_STATIC_INVALID_OPTION_VALUE('list.names', 'must be an array')
    }
    if (options.list.jsonFormat != null && options.list.jsonFormat !== 'names' && options.list.jsonFormat !== 'extended') {
      return new FST_STATIC_INVALID_OPTION_VALUE('list.jsonFormat', 'must be names or extended')
    }
    if (options.list.format === 'html' && typeof options.list.render !== 'function') {
      return new FST_STATIC_INVALID_OPTION_VALUE('list.render', 'must be a function and is required with html format')
    }
  }
}
